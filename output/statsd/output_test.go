...
func TestStatsdv2Output(t *testing.T) {
	t.Parallel()
	tagMap := stats.TagSet{"tag1": true, "tag2": true}
	tagFormats := []string{"statsdv2", "datadog", "newrelic"}
	for _, tagFormat := range tagFormats { //nolint:paralleltest
		tagFormat := tagFormat
		...
		t.Run(tagFormat, func(t *testing.T) {
			...
			"tagFormat": "%s"
		}`, addr.String, namespace.String, bufferSize.Int64, pushInterval.Duration.String(), tagFormat)),
					})
